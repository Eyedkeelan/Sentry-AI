...
if Training: 
  ...
  for epoch in range(epochs):
      ...
      if best_loss > losses[1, epoch]:
          ...
          torch.save(model_binary.state_dict(), os.path.join("/mnt/scratch/od22kob/MSC Final/Sentry-AI/Python/Models/Best_Models", "TimeSformer.pt"))
...
# %%
model_binary.load_state_dict(torch.load(os.path.join("/mnt/scratch/od22kob/MSC Final/Sentry-AI/Python/Models/Best_Models", "TimeSformer.pt")))
...
def get_predictions(model, dataloader, device):
    ...
    with torch.no_grad():
        for frames, labels in tqdm(dataloader, desc="Getting predictions"):
            ...
            outputs = model(frames).logits  # Raw output (before sigmoid)
            probs = torch.sigmoid(outputs.squeeze())  # Convert to probability

            # Ensure shapes match up (handle batch size 1 as well)
    ...
    return np.array(preds_list), np.array(probs_list), np.array(true_list)
# Example usage:
device = torch.device(device)  # Change to "cuda" if using GPU
pred_labels, probs, true_labels = get_predictions(model_binary, testing_dataloader, device)
