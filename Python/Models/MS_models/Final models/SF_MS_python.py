...
metadata['Anomaly Type']
print(set(anomaly),len(set(anomaly)))
...
classes_number = len(set(anomaly))
...
class FrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        frame_labels = video_frames["Anomaly Type"].tolist()
...
num_workers = 12 #Set to 0 if on local machine. 
if Training: 
...
testing_dataloader = torch.utils.data.DataLoader(testing_dataset, batch_size=8, num_workers=num_workers, pin_memory=True)
...
# %%
slowfast_model = torch.hub.load('facebookresearch/pytorchvideo', 'slowfast_r101', pretrained=True)

# Remove classification head
slowfast_model.blocks[-1] = nn.Identity() # Remove the final classification layer

class SlowFastMSClassifier(nn.Module):
    def __init__(self, slowfast_model, classes_number = classes_number):
        super(SlowFastMSClassifier, self).__init__()
        self.slowfast_model = slowfast_model
        self.fc = nn.Linear(2304 , classes_number)  # Output number of classes (12 in multi scenario) 
...
model_ms = SlowFastMSClassifier(slowfast_model)

epochs = 50
losses = np.zeros((2, epochs))
# Wrap to add parallel processing 
...
if Training: 
  ...
  for epoch in range(epochs):
      ...
      for frames, labels in tqdm(training_dataloader, desc=f"Training pass epoch: {epoch}"):
          ...
          pred = model_ms(slow_frames, fast_frames)
          loss = loss_function(pred.squeeze(), labels.long())
      ...
      with torch.no_grad():
          for test_frames, test_labels in tqdm(testing_dataloader, desc="Cycling Testing Dataloader"):
              ...
              test_preds = model_ms(slow_test_frames, fast_test_frames)  
              t_loss = loss_function(test_preds.squeeze(), test_labels.long())
      ...
      if best_loss > losses[1, epoch]:
          ...
          torch.save(model_ms.state_dict(), os.path.join("/mnt/scratch/od22kob/MSC Final/Sentry-AI/Python/Models/Best_Models", "E2E_SF_MS.pt"))
  
      print(f"Epoch [{epoch+1}/{epochs}] - Training Loss: {losses[0,epoch]:.4f}, Test Loss: {losses[1,epoch]:.4f}")
      scheduler.step(losses[1, epoch])
  ...
  plt.ylabel("Loss")
  plt.legend()
  plt.title("Slow-Fast Multi-Scenario Classification Model")
  plt.savefig("Training_E2E_SF_MS.png")
  plt.show()

# %%
# Define the file path
model_path = os.path.join("/mnt/scratch/od22kob/MSC Final/Sentry-AI/Python/Models/Best_Models", "E2E_SF_MS.pt")

# Load the state_dict directly from the file path
...
# Reinitialize the model and load the state_dict
model_ms = SlowFastMSClassifier(slowfast_model)
model_ms.load_state_dict(state_dict)
...
class_names = l_encoder.classes_
#device = torch.device(device) 
true_labels, pred_labels = get_predictions(model_ms.to(device), testing_dataloader, device)
...
cm = metrics.confusion_matrix(true_labels, pred_labels, normalize='true')
...
plt.tight_layout()
plt.savefig('ms_sf_cm_final.png')
plt.show()


report = metrics.classification_report(true_labels, pred_labels, target_names=class_names)

print(report)


report_path = "classification_report_ms_sf.txt"
with open(report_path, "w") as f:
    f.write("Classification Report for Slow-Fast MS Classifier\n\n")
    f.write(report)
