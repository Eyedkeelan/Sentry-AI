...
# %%
project_path = r"/mnt/scratch/od22kob/MSC Final/" #Path to project destination
...
batch_size = 16 if device.type == 'cuda' else 8  # Larger batch size for GPU 15/04/2025: Reduced batch size to 16
...
if Training:
  for epoch in range(epochs):
      ...
      with torch.no_grad():
          for test_frames, test_labels in tqdm(testing_dataloader, desc="Validating"):
              ...
              t_loss = loss_function(test_preds, test_labels.long())
      ...
      if best_loss > losses[1, epoch]:
          ...
          torch.save(model_ms.state_dict(), os.path.join("/mnt/scratch/od22kob/MSC Final/Sentry-AI/Python/Models/Best_Models/", "TimeSformer_ms.pt"))
  ...
  plt.show()
# %%
model_ms.load_state_dict(torch.load(os.path.join("/mnt/scratch/od22kob/MSC Final/Sentry-AI/Python/Models/Best_Models/", "TimeSformer_ms.pt")))
...
def get_predictions(model, dataloader, device):
    ...
    with torch.no_grad():
        for frames, labels in tqdm(dataloader, desc="Getting Predictions"):
            frames = frames.to(device)
            labels = labels.to(device)

            outputs = model(frames).logits  # (batch_size, num_classes)
...
cm_normalized = metrics.confusion_matrix(true_labels, pred_labels, normalize='true')
sns.heatmap(cm_normalized, annot=True, fmt=".2f", cmap="viridis",
            xticklabels=class_names, yticklabels=class_names)
plt.title("Normalized Confusion Matrix of Multi-scenario Transformer Classifier")
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.xticks(rotation=45, ha="right")
plt.yticks(rotation=0)
plt.tight_layout()
plt.savefig('ms_tf_cm_final.png')
plt.show()
# %%
...
report_path = "classification_report_ms_tf.txt"
with open(report_path, "w") as f:
    f.write("Classification Report for VideoMAE MS Classifier\n\n")
    f.write(report)
