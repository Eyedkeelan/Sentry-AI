...
testing_dataloader = torch.utils.data.DataLoader(testing_dataset, batch_size=8, shuffle=True, num_workers=0)#, pin_memory=True)
...
# %%
model_binary = SlowFastMSClassifier(slowfast_model)
...
optimiser = optim.Adam(model_ms.parameters(), lr=1e-7, weight_decay=1e-4)  
...
# %%
model_binary.load_state_dict(torch.load(os.path.join("Best_Models", "E2E_SF_MS.pt")))
...
plt.ylabel("Actual")
plt.savefig("E2E_SF_MS_results.png")
plt.show()
